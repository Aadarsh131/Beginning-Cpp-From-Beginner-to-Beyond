...
void test2() {
	...
	std::cout << "\nvec size: " << vec.size() << std::endl;
	std::cout << "vec max size: " << vec.max_size() << std::endl;
	std::cout << "vec capacity: " << vec.capacity() << std::endl; //increases by 2 (google for more)
	...
	vec.reserve(100); //allocates space of 100, and until the space of 100 is full, that the capacity will not be changed
	display(vec);
	std::cout << "\nvec size: " << vec.size() << std::endl;
	std::cout << "vec max size: " << vec.max_size() << std::endl;
	std::cout << "vec capacity: " << vec.capacity() << std::endl;
...
void test4() {
	...
	stooges.push_back(p1); //p1 here is l-value, hence a copy will be stored, using the default copy constructor
	display(stooges);

	stooges.push_back(Person{ "Moe", 25 }); //unamed(we are unable to reference it will any name) object, meaning ite an r-value, hence the move semantics will take place, and this object will be moved directly to the back of vector 
	display(stooges);
...
void test7() {
	...
	vec2.swap(vec1); //size of the vectors will be adjusted as per the size
	display(vec1);
	display(vec2);
}
